...
static float mass = 2.632f; // [kg]
static float inertia = 0.0f; // not correct, should be changed to correct value if rotational inertia feedforward will also be implemented
...
int stateControl_Init() {
	...
	feedforwardParameters.param1[0] = 0.46f;
	feedforwardParameters.param1[1] = 0.46f;
	feedforwardParameters.param1[2] = 0.46f;
	feedforwardParameters.param1[3] = 0.46f;

	feedforwardParameters.param2[0] = 0.0360f * 180.0f / M_PI;
	feedforwardParameters.param2[1] = 0.0360f * 180.0f / M_PI;
	feedforwardParameters.param2[2] = 0.0350f * 180.0f / M_PI;
	feedforwardParameters.param2[3] = 0.0354f * 180.0f / M_PI;

	feedforwardParameters.param3[0] = -0.764611677882548f;
	feedforwardParameters.param3[1] = -2.33703565345706f;
	feedforwardParameters.param3[2] = -1.00311434719396f;
	feedforwardParameters.param3[3] = 4.16457002728926f;

	feedforwardParameters.param4[0] = 1.23f;
	feedforwardParameters.param4[1] = 1.23f;
	feedforwardParameters.param4[2] = 1.23f;
	feedforwardParameters.param4[3] = 1.23f;
...
void stateControl_Update(ControlOutput* const output) {
	if (status == on) {
		stateControl_Update_Body();
		wheels_Update(output);
	}
}
 ...
 */
void wheels_Update(ControlOutput* const output) {
	...
	for (motor_id_t motor = RF; motor <= RB; motor++) {
		...
		if (fabs(wheelRef[motor]) < wheel_speed_threshold) {
			...
			feed_forward[motor] = wheelFFOn * (feedforwardParameters.identified_damping * wheelRef[motor] +
				feedforwardFriction(wheelRef[motor], rho, theta_local, omega, motor)) +
				massFFOn * feedforwardMass(stateLocalRefAcceleration, motor);
		}
		...
		if (fabs(angular_velocity_error) < 0.1f) {
			angular_velocity_error = 0.0f;
			wheelsK[motor].I = 0.0f;
		}

		float feed_back_voltage = wheelFBOn * 24.0f * (0.001367311f * (PID(angular_velocity_error, &wheelsK[motor])));
	...
	memcpy(&output->wheel_efforts, wheel_speed_fraction, 16);
}
...
static float feedforwardFriction(float wheelRef, float rho, float theta, float omega, wheel_names wheel) {
	...
	float gamma = 0.5f - 0.5f * cos(M_PI * fabs(omega / vw_max_round_to_rotational));
...
static float absoluteAngleControl(float angleRef, float angle) {
	...
	if (angleErr == 0) {
		angleErr = 0.000001f * prevangleErr;
	}
