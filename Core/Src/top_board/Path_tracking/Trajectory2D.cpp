...
#include "../top_board/Path_tracking/Trajectory2D.h"
#include "../top_board/Path_tracking/Print.h"
...
Vector2 Trajectory2D::getLastAcceleration(int robotId) {
    // Check if robotId exists in the map using find()
    auto it = Trajectory2D::lastAcceleration.find(robotId);
    if (it != Trajectory2D::lastAcceleration.end()) {
        return it->second; // Return the corresponding value
    } else {
        return Vector2(0, 0); // Return default acceleration if not found
    }
}
